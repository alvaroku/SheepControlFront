...
function update(id) {
    ...
    updateForm.permissionId.value = objToUpdate.permission.id
        ...
        function toggleActive(event,id){
            ...
            fetchRequest(urlPermissionRole+"toggleActive/" + id, {headers: {'Content-Type': 'application/json','Accept': 'application/json',"Authorization": `Bearer ${getCookie('auth')}`}}, function (error, data) {
                if (error) {
                    showMessage("error","Mensaje","Error al actualizar")
                    ...
                    tr.innerHTML = createPermissionRoleTds(dataToogle)
