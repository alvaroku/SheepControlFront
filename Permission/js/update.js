...
function update(id) {
    ...
    updateForm.actionId.value = objToUpdate.action.id
        ...
        function toggleActive(event,id){
            ...
            fetchRequest(urlPermission+"toggleActive/" + id, {headers: {'Content-Type': 'application/json','Accept': 'application/json',"Authorization": `Bearer ${getCookie('auth')}`}}, function (error, data) {
                if (error) {
                    showMessage("error","Mensaje","Error al actualizar")
                    ...
                    tr.innerHTML = createPermissionTds(dataToogle)
