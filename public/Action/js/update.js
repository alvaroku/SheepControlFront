...
function update(id) {
    ...
    updateForm.name.value = objToUpdate.name
        ...
        function toggleActive(event,id){
            ...
            fetchRequest(urlAction+"toggleActive/" + id, {headers: {'Content-Type': 'application/json','Accept': 'application/json',"Authorization": `Bearer ${getCookie('auth')}`}}, function (error, data) {
                if (error) {
                    showMessage("error","Mensaje","Error al actualizar")
